...
import { Proveedor, IContactoProveedor } from "../../types";
...
const ProveedorList: React.FC = () => {
  ...
  const columns: DataTableColumn<Proveedor>[] = [
    ...
    { key: "contactos", label: "Contactos", render: (p) => (
      <ul className="mb-0 ps-3">
        {p.contactos.map((c, i) => (
          <li key={i}>
            <b>{c.nombre}</b> ({c.puesto})<br/>
            {c.correo}, {c.telefono}{c.extension ? `, ext. ${c.extension}` : ""}
          </li>
        ))}
      </ul>
    ) },
  ...
  return (
    <div className="container-fluid mt-4 px-1 px-sm-3">
      <div className="d-flex flex-column flex-md-row justify-content-between align-items-stretch gap-2 mb-3">
        ...
        />
        <Button variant="success" onClick={() => { setShowModal(true); setEditId(null); setNewProveedor({ ...emptyProveedor }); }}>
          Agregar Proveedor
        </Button>
      </div>
        ...
        <DataTable
          ...
          actions={(proveedor) => (
            <div className="d-flex flex-column flex-sm-row align-items-stretch gap-1">
              <Button variant="warning" size="sm" className="w-100 w-sm-auto" onClick={() => handleEdit(proveedor)}>Editar</Button>
              <Button variant="danger" size="sm" className="w-100 w-sm-auto" onClick={() => handleDelete(proveedor._id!)}>Eliminar</Button>
            </div>
          )}
          className="small"
                ...
                <Row className="align-items-end">
                  <Col md={6} sm={6} xs={12} className="mb-2 mb-md-0">
                    <Form.Control 
                      ...
                      placeholder="Nombre" 
                      value={c.nombre} 
                      onChange={e => setNewProveedor({ 
                        ...newProveedor, 
                        contactos: newProveedor.contactos.map((x, i) => i === idx ? { ...x, nombre: e.target.value } : x) 
                      })} 
                    />
                  </Col>
                  <Col md={5} sm={5} xs={10} className="mb-2 mb-md-0">
                    <Form.Control 
                      ...
                      onChange={e => setNewProveedor({ 
                        ...newProveedor, 
                        contactos: newProveedor.contactos.map((x, i) => i === idx ? { ...x, puesto: e.target.value } : x) 
                      })} 
                    />
                  </Col>
                ...
                <Row className="align-items-end mt-2">
                  <Col md={4} sm={12} xs={12} className="mb-2 mb-md-0">
                    <Form.Control 
                      ...
                      onChange={e => setNewProveedor({ 
                        ...newProveedor, 
                        contactos: newProveedor.contactos.map((x, i) => i === idx ? { ...x, correo: e.target.value } : x) 
                      })} 
                    />
                  </Col>
                  <Col md={4} sm={6} xs={6} className="mb-2 mb-md-0">
                    <Form.Control 
                      ...
                      onChange={e => setNewProveedor({ 
                        ...newProveedor, 
                        contactos: newProveedor.contactos.map((x, i) => i === idx ? { ...x, telefono: e.target.value } : x) 
                      })} 
                    />
                  </Col>
                  <Col md={4} sm={6} xs={6} className="mb-2 mb-md-0">
                    <Form.Control 
                      ...
                      onChange={e => setNewProveedor({ 
                        ...newProveedor, 
                        contactos: newProveedor.contactos.map((x, i) => i === idx ? { ...x, extension: e.target.value } : x) 
                      })} 
                    />
                  </Col>
                </Row>
